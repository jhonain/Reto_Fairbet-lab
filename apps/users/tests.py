...
class RegistroUsuarioApiTests(TestCase):
    ...
    def datos_registro(self, username='nuevo', dni=None, fecha_nacimiento='2000-01-01'):
        return {'username': username, 'password': 'clave-segura-123', 'dni': dni or generar_dni_valido(), 'fecha_nacimiento': fecha_nacimiento}
    ...
    def test_registro_api_exitoso(self):
        ...
        self.assertTrue(Cuenta.objects.filter(usuario=user, tipo_cuenta=TipoCuenta.BILLETERA_USUARIO).exists())
    ...
    def test_registro_api_con_menor_de_edad_devuelve_400(self):
        ...
        response = self.post_json(self.datos_registro(username='menor', fecha_nacimiento=menor_de_edad))
        self.assertEqual(response.status_code, 400)
...
class RegistroUsuarioWebTests(TestCase):
    ...
    def datos_registro(self, username='webnuevo', dni=None, fecha_nacimiento='2000-01-01'):
        return {'username': username, 'password': 'clave-segura-123', 'dni': dni or generar_dni_valido('7654321'), 'fecha_nacimiento': fecha_nacimiento}

    def test_registro_web_con_dni_con_letras_no_crea_usuario(self):
        response = self.client.post(self.url, data=self.datos_registro(username='dni_letras', dni='RRRRRRRR'))
    ...
    def test_registro_web_con_menor_de_edad_no_crea_usuario(self):
        menor_de_edad = date.today().replace(year=date.today().year - 17).isoformat()
        response = self.client.post(self.url, data=self.datos_registro(username='web_menor', fecha_nacimiento=menor_de_edad))
        self.assertEqual(response.status_code, 200)
    ...
    def test_registro_web_correcto_crea_usuario_y_perfil(self):
        response = self.client.post(self.url, data=self.datos_registro())
