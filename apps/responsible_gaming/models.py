...
class AutoExclusion(models.Model):
    ...
    tipo = models.CharField(
        ...
        verbose_name="Tipo de exclusion"
    )
...
class LimiteDeposito(models.Model):
    ...
    pendiente_desde = models.DateTimeField(
        null=True,
        blank=True,
        verbose_name="Pendiente desde"
    )
    pendiente_aplicable_desde = models.DateTimeField(
        null=True,
        blank=True,
    ...
    enfriamiento_hasta = models.DateTimeField(
        null=True,
        blank=True,
        verbose_name="Enfriamiento hasta"
    )
    ...
    def aplicar_aumento_pendiente(self):
        ...
        if not self.puede_aplicar_aumento_pendiente():
            raise ValidationError(
                f"El aumento pendiente estara disponible desde: "
                f"{self.pendiente_aplicable_desde.strftime('%Y-%m-%d %H:%M')}"
            )
