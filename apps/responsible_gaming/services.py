...
def validar_limite_recarga(usuario, monto: Decimal):
    if obtener_autoexclusion_vigente(usuario):
        raise ValidationError(
            "No puedes recargar porque tienes una autoexclusion vigente."
        )
...
def registrar_alerta_multiples_cuentas_ip(usuario, ip_address):
    ...
    registro, _ = SuspiciousActivity.objects.get_or_create(
        usuario=usuario,
        regla=REGLA_REGISTRO_IP,
        ip_address=ip_address,
        defaults={
            "descripcion": "Registro de usuario asociado a una direccion IP.",
            "metadata": {"usuario_id": usuario.id},
            "estado": "revisado",
        },
    )

    usuarios_en_ip = SuspiciousActivity.objects.filter(
        regla=REGLA_REGISTRO_IP,
        ip_address=ip_address,
    ...
    alerta_existente = SuspiciousActivity.objects.filter(
        usuario=usuario,
        regla=REGLA_MULTIPLES_CUENTAS_IP,
        ip_address=ip_address,
    ).exists()
    if alerta_existente:
        return registro
    ...
    return SuspiciousActivity.objects.create(
        ...
        regla=REGLA_MULTIPLES_CUENTAS_IP,
        descripcion=(
            "Se detectaron varias cuentas registradas desde la misma direccion IP."
        ),
        ip_address=ip_address,
        metadata={
            "usuarios_en_ip": usuarios_en_ip,
            "umbral": UMBRAL_MULTIPLES_CUENTAS_IP,
        },
    )
