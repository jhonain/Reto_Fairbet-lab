...
class ApuestaSimpleTests(TestCase):
    ...
    def setUp(self):
        ...
        self.evento = Evento.objects.create(nombre='Demo', equipo_local='A', equipo_visitante='B', inicia_en=inicio, estado=EstadoEvento.PROGRAMADO)
        self.mercado = Mercado.objects.create(evento=self.evento, tipo=TipoMercado.UNO_X_DOS)
        self.cuota = Cuota.objects.create(mercado=self.mercado, seleccion=CodigoSeleccion.LOCAL, valor=Decimal('2.5000'))

    def test_apuesta_aceptada_y_payout(self):
        apuesta = crear_apuesta_simple(self.user, self.cuota.id, MONTO_MINIMO, 'clave-test-1', True)
