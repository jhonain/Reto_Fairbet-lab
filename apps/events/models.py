...
# ⚡ IMPORTS EXCLUSIVOS DE TU PARTE PARA TIEMPO REAL (NIVEL 1 Y 2)
...
# =====================================================================
# 👥 ENTIDAD 1: EVENTO (ESTRUCTURA UNIFICADA)
# =====================================================================
class Evento(models.Model):
    ...
    resultado = models.JSONField(
        null=True,
        blank=True,
        help_text='{"local": 2, "visitante": 1}',
        verbose_name="Resultado final"
    )
    
    # Campos para control de apuestas en vivo (In-play)
    es_momento_critico = models.BooleanField(
        default=False,
        verbose_name="¿Momento crítico?"
    )
    momento_critico_desde = models.DateTimeField(
        null=True,
        blank=True,
        verbose_name="Momento crítico desde"
    )
    ultimo_cambio_estado = models.DateTimeField(
        auto_now_add=True,
        verbose_name="Último cambio de estado"
    )
    apuestas_cerradas_en = models.DateTimeField(
        null=True,
        blank=True,
        verbose_name="Apuestas cerradas en"
    )
    
    # Variación de equipo (si aplica)
    desempate_fair_play = models.JSONField(
        null=True,
        blank=True,
        help_text='{"tarjetas_amarillas_local": 2, "tarjetas_rojas_local": 0, ...}',
        verbose_name="Desempate fair play"
    )
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=["estado", "inicia_en"]),
            models.Index(fields=["es_momento_critico", "momento_critico_desde"]),
        ]
    ...
    # 🚀 TU MOTOR DEL NIVEL 2: CONTROL DE CONCURRENCIA E HILOS ASÍNCRONOS
    def suspender_por_evento_critico(self, segundos=30):
        """
        Bloqueo inteligente de mercados in-play para evitar hilos huérfanos.
        """
        ...
        # Suspende de golpe todos los mercados asociados que estén abiertos
        ...
        # Hilo secundario daemon para liberar la UI automáticamente sin bloquear a los usuarios
...
# =====================================================================
# 👥 ENTIDAD 2: MERCADO (ESTRUCTURA ORIGINAL DEL EQUIPO)
# =====================================================================
class Mercado(models.Model):
    ...
    suspendido_hasta = models.DateTimeField(
        null=True,
        blank=True,
        verbose_name="Suspendido hasta"
    )
    motivo_suspension = models.CharField(
        max_length=20,
        choices=MotivoSuspension.choices,
        null=True,
        blank=True,
        verbose_name="Motivo de suspensión"
    )
    suspendido_por = models.CharField(
        max_length=20,
        choices=[("SISTEMA", "Sistema"), ("OPERADOR", "Operador")],
        null=True,
        blank=True,
    ...
    margen_operador = models.DecimalField(
        ...
        help_text="Margen de beneficio del operador, ej. 0.05 = 5%",
        verbose_name="Margen del operador"
    )
    ...
    def reabrir(self):
        """Reabrir mercado después de suspensión."""
...
# =====================================================================
# 👥 ENTIDAD 3: CUOTA (PRECIOS DINÁMICOS)
# =====================================================================
class Cuota(models.Model):
    ...
    seleccion = models.CharField(
        max_length=15,
        choices=CodigoSeleccion.choices,
        verbose_name="Selección"
    )
    valor = models.DecimalField(
        max_digits=10,
        decimal_places=4,
        help_text="Formato decimal europeo, ej. 2.5000",
...
# =====================================================================
# 👥 ENTIDAD 4: HISTORIAL CUOTA (AUDITORÍA INMUTABLE)
# =====================================================================
class HistorialCuota(models.Model):
    """Historial inmutable de cambios de cuotas para auditoría."""
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
...
# =====================================================================
# ⚡ TU SISTEMA DE DISPARADORES EN TIEMPO REAL (SIGNALS ASÍNCRONOS)
# =====================================================================
