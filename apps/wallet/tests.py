...
def _dni_valido():
    """Genera un DNI de 8 dígitos con checksum correcto (para tests)."""
    base = "1234567"
...
class ServicioBilleteraTests(TestCase):
    ...
    def test_bloqueo_apuesta_descuenta_saldo(self):
        ...
        ServicioBilletera.bloquear_para_apuesta(
            self.user, Decimal("40.0000"), id_ap, uuid.uuid4()
        )
        cuenta = Cuenta.objects.get(usuario=self.user, tipo_cuenta=TipoCuenta.BILLETERA_USUARIO)
...
class ConcurrenciaRecargaTests(TransactionTestCase):
    ...
    def _retirar(self, monto, clave):
        # Cada hilo necesita su propia conexión en Django
        connection.close()
        try:
    ...
    @unittest.skipIf(
        ...
        "Concurrencia real se prueba en PostgreSQL (docker)",
    )
    def test_no_doble_gasto_en_retiros_paralelos(self):
        clave1, clave2 = uuid.uuid4(), uuid.uuid4()
        with ThreadPoolExecutor(max_workers=2) as pool:
            futs = [
                pool.submit(self._retirar, Decimal("80.0000"), clave1),
                pool.submit(self._retirar, Decimal("80.0000"), clave2),
            ]
