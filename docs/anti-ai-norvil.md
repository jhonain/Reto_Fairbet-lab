...
| 2026-05-28 | `templates/portal/eventos.html` | Generar UI | Script JS para abrir el WebSocket, escuchar mensajes `estado_mercado` y manipular el DOM (botón rojo) sin recargar página. |
...
| 2026-05-29 | `apps/events/management/commands/seed_eventos.py` | Adaptar datos | Apoyo en la reducción de longitud de strings para los identificadores de cuotas del Nivel 2 debido a restricciones físicas en las columnas. |
...
- **Jerarquía Multi-Mercado:** Diseñé la estructuración del contexto en `apps/portal/views.py` mediante consultas optimizadas (`prefetch_related`) para desacoplar el listado de eventos y permitir la carga simultánea de mercados avanzados (Over/Under, BTTS, Hándicap).
...
### 2026-05-28 / 2026-05-29 — Implementación de Tiempo Real, Suspensión y Catálogo Avanzado (Nivel 2)
...
| 2026-05-29 | Depuración de excepciones SQL | Corregir | La IA asistió descifrando el log `DataError: value too long` de Postgres, recomendando compactar los códigos de selección a menos de 15 caracteres. |
| 2026-05-29 | Documentación `docs/anti-ia-norvil.md` | Redactar declaración | Basada en el formato de los compañeros de equipo; adapté mi bitácora de soporte técnico de hoy. |
...
- `feat(events): implementar suspension automatica de mercados in-play por 30s y bloqueo en tiempo real via websockets [ai-assisted]`